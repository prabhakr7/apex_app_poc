...
-- changeset PRLNU:1753338759448 stripComments:false  logicalFilePath:milestone-1/prlnu/package_bodies/eba_cust_email.sql runAlways:false runOnChange:false replaceIfExists:true failOnError:true
-- sqlcl_snapshot src/database/prlnu/package_bodies/eba_cust_email.sql:null:7dec399290f28fcd69d2ba18dee5fdc6b0543ed6:create
...
'; -- Email Bottom
    ...
    begin
        for c1 in (
            ...
            where
                    upper(build_option_name) = upper(p_bo_name)
    ...
    begin
        ...
        m := m
            ...
            replace(
                replace(c_top_tmpl,
                        '#APP_NAME#',
                        eba_cust_fw.get_preference_value('APPLICATION_TITLE')),
                '#SUBJECT#',
                c_fb_subject
            ),
            '#RECIPIENT_EMAIL#',
        ...
        l_body := '<p>'
                  || l_submitter
                  || ' has submitted the following <strong><a href="'
                  || c_base_url
                  || apex_util.prepare_url('f?p='
                                           ...
                                           || ':0:::'
                                           || p_fb_rpt_page
                                           || ',RIR:IREQ_PAGE_ID,IREQ_TYPE_ID,IREQ_CREATED_BY:'
                                           || p_page_id
                                           || ','
                                           || l_feedback_type
                                           || ','
    ...
    procedure send_feedback_email (
        ...
        l_from       varchar2(255);
    begin
        if eba_cust_fw.get_preference_value(p_preference_name => 'FEEDBACK_RECIPIENTS') = 'Preference does not exist'
        or get_build_option_status(
            p_app_id  => p_app_id,
            p_bo_name => 'Feedback'
        ) = 'EXCLUDE' then
        ...
        if instr(p_submitter, '@') > 0 then
            ...
            for c1 in (
                ...
                from
                    eba_cust_users
                where
                        upper(username) = upper(p_submitter)
        ...
        if l_from is null then
            if instr(
                eba_cust_fw.get_preference_value(p_preference_name => 'FEEDBACK_RECIPIENTS'),
                ','
            ) > 0 then
                l_from := lower(substr(
                    eba_cust_fw.get_preference_value(p_preference_name => 'FEEDBACK_RECIPIENTS'),
